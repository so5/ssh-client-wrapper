...
const debug = Debug("sshClientWrapper:debug:util");

export const sshCmd = "ssh";
export const rsyncCmd = "rsync";
export const acceptableRsyncRetrunCodes = [10, 11, 12, 13, 14];

export const rePwPrompt = /password:/;
export const rePhPrompt = /Enter passphrase for key/;
export const reNewHostPrompt = /Are you sure you want to continue connecting/;
export const reRemoteHostIdentificationHasChanged = /WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED! /;
...
const hostInfoSchema = {
  ...
  properties: {
    ...
    replaceRetryableExitCodes: { type: "boolean" },
    rcfile: { type: "string", pattern: "\\S+", transform: ["trim"] },
    prependCmd: { type: "string", pattern: "\\S+", transform: ["trim"] },
    sshOpt: {
      type: "array",
      minItems: 1,
      items: { type: "string", pattern: "\\S+", transform: ["trim"] }
    },
    useAgent: { type: "boolean" },
    identityAgent: { type: "string", pattern: String.raw`\S+`, transform: ["trim"] },
    agentKeyTTL: { type: "number", minimum: 1 }
  },
  required: ["host"]
};
 ...
 */
export const buff2String = (func, data)=>{
 ...
 */
export const sanityCheck = (hostInfo)=>{
  ...
  if (typeof hostInfo.identityAgent === "string" && /\s/.test(hostInfo.identityAgent)) {
    const err = new Error(`invalid identityAgent specified ${hostInfo.identityAgent}`);
    err.hostInfo = hostInfo;
    throw err;
  }
 ...
 */
export const getSshOption = (hostInfo, withoutDestination)=>{
  ...
  //Enable SSH agent forwarding if keyFile is set (indicates we're using key-based auth)
  if (hostInfo.keyFile) {
    args.push("-A");
  }

  //route auth through a wrapper-managed or caller-supplied ssh-agent when one is in play
  const agentSock = hostInfo.managedAgentSock || hostInfo.identityAgent;
  if (typeof agentSock === "string" && agentSock !== "") {
    args.push(`-oIdentityAgent=${agentSock}`);
  }
...
export const isArrayOfInteger = (target)=>{
  if (!Array.isArray(target)) {
    return false;
  }
  return !target.some((e)=>{
    //target has non integer member
    return !Number.isInteger(e);
  });
};

/**
 * resolve which rsync exit codes should be treated as retryable for a single send/recv call
 * @param {object} hostInfo - hostinfo object
 * @param {number[]} methodCodes - retryableExitCodes argument passed directly to send/recv (highest priority)
 * @param {boolean} methodReplace - replaceRetryableExitCodes argument passed directly to send/recv
 * @returns {number[]} - exit codes which should be treated as retryable
 *
 * precedence: method-level argument (if given) fully supersedes hostInfo-level settings.
 * at whichever level is used, replace=false (default) merges with acceptableRsyncRetrunCodes,
 * replace=true uses the given codes verbatim (so [] disables exit-code-based retry entirely).
 */
export const resolveRetryableExitCodes = (hostInfo, methodCodes, methodReplace)=>{
  if (methodCodes != null) {
    return methodReplace ? methodCodes : [...acceptableRsyncRetrunCodes, ...methodCodes];
  }
  if (hostInfo.retryableExitCodes != null) {
    return hostInfo.replaceRetryableExitCodes
      ? hostInfo.retryableExitCodes
      : [...acceptableRsyncRetrunCodes, ...hostInfo.retryableExitCodes];
  }
  return acceptableRsyncRetrunCodes;
};
 ...
 */
export const sendPty = async (pty, message, logger, label)=>{
 ...
 */
export const watchDogTimer = (timeout, label, cb)=>{
