...
const debugMasterVerbose = require("debug")("sshClientWrapper:insecure:fork_masterPty");
const debugSendPty = require("debug")("sshClientWrapper:insecure:fork_sendpty");
 ...
 */
function getMasterPty(hostInfo, logger) {
  ...
  ptyProcess.onData(buff2String.bind(null, debugMasterVerbose));
  ptyProcess.onData((data)=>{
    const output = data.toString();
    sshLoginCallback(output, ptyProcess, hostInfo.password, hostInfo.passphrase, logger);
  });
  hostInfo.masterPty = ptyProcess;
}

async function fork(hostInfo, cmd, args, timeout, outputCallback) {
  return new Promise((resolve, reject)=>{
    ...
    if (timeout > 0) {
      ac = watchDogTimer(timeout, cmd, ()=>{
        ptyProcess.kill();
        reject(new Error(`watchdog timer expired ${timeout}`));
      });
    }
