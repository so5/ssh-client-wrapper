...
class SshClientWrapper {
  ...
  async send(src, dst, opt = {}) {
    debug("send called", src, dst);

    if (!isArrayOfString(src)) {
      return new Error("src must be array of string");
    }
    if (typeof dst !== "string") {
      return new Error("dst must be string");
    }
    if (!isArrayOfString(opt)) {
      return new Error("opt must be array of string");
    }
