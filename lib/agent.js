...
function resolveWellKnownDir() {
  ...
  candidates.push(path.join(os.tmpdir(), `scw-${uid}`));
  candidates.push(`/tmp/scw-${uid}`);
...
async function ensureAgentForHost(hostInfo) {
  ...
  const ttl = Number.isFinite(hostInfo.agentKeyTTL) && hostInfo.agentKeyTTL > 0
  ...
  let candidate = null;
  if (typeof hostInfo.identityAgent === "string" && hostInfo.identityAgent !== "") {
    candidate = hostInfo.identityAgent;
  } else if (typeof process.env.SSH_AUTH_SOCK === "string" && process.env.SSH_AUTH_SOCK !== "") {
    candidate = process.env.SSH_AUTH_SOCK;
  } else if (!canSpawnSharedAgent() && process.platform === "win32") {
    candidate = AGENT_DEFAULT;
  } else if (!canSpawnSharedAgent()) {
    return;
  }
  ...
  try {
    if (hasKey) {
      await ensureKeyLoaded(hostInfo, candidate, ttl);
    } else {
      await ensurePreloadedAgent(hostInfo, candidate);
    }
  } catch (e) {
