...
async function send(hostInfo, argSrc, dst, rsyncOpt, timeout) {
  ...
  return retryWrapper(fork.bind(null, hostInfo, rsyncCmd, args, timeout), hostInfo);
}
 ...
 * @param {Object} hostInfo - hostinfo object
 * @param {[string]} argSrc - File or directory name which to be send.
 * @returns {string} - src argument for rsync
 */
async function formatSrc(hostInfo, argSrc) {
  ...
  if (hostInfo.rsyncVersion.major >= 3 && hostInfo.rsyncVersion.minor >= 2 && hostInfo.rsyncVersion.patch >= 4) {
    return argSrc.map((e)=>{
      return `${hostInfo.host}:${e}`;
    }).join(" ");
  }
  return `${hostInfo.host}:${argSrc.join(" ")}`;
}
...
async function recv(hostInfo, argSrc, dst, rsyncOpt, timeout) {
  await connect(hostInfo);
  const src = await formatSrc(hostInfo, argSrc);
  const sshArgs = getSshOption(hostInfo, true);
  const rshOpt = ["-e", `${sshCmd} ${sshArgs.join(" ")}`];
