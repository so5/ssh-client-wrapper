...
import { sendPty, sshCmd, getSshOption, sanityCheck, watchDogTimer, rePwPrompt, rePhPrompt } from "./util.js";
import { fork, createMasterPty, sshLoginCallback } from "./fork.js";
import { shouldUseAgent, ensureAgentForHost } from "./agent.js";

const debug = Debug("sshClientWrapper:debug:sshExec");
const debugVerbose = Debug("sshClientWrapper:verbose:sshExec");
const debugSendMasterPty = Debug("sshClientWrapper:insecure:sshExec:masterPty");
 ...
 */
export async function sshExec(hostInfo, cmd, timeout, outputCallback, rcfile, prependCmd) {
...
export async function expect(hostInfo, cmd, expects, timeout) {
  debug("expect called");
  await connect(hostInfo);
 ...
 */
export async function canConnect(hostInfo, timeout) {
 ...
 * @param {object} hostInfo - hostinfo object
 */
export async function connect(hostInfo, timeout = 60) {
  ...
  if (shouldUseAgent(hostInfo)) {
    try {
      ...
      debug("agent setup failed; falling back to legacy pty auth", e);
  ...
  const p = new Promise((resolve, reject)=>{
    ...
    hostInfo.masterPty.onData((data)=>{
      ...
      if (shouldUseAgent(hostInfo) && (rePwPrompt.test(output) || rePhPrompt.test(output))) {
        if (typeof hostInfo.reauthRequired === "function") {
          try {
            hostInfo.reauthRequired();
          } catch {
            //a consumer hook must never break the connection
          }
        }
        if (hostInfo.password == null && hostInfo.passphrase == null) {
          const err = new Error("agent authentication failed and no secret is available for fallback");
 ...
 * @param {object} hostInfo - hostinfo object
 */
export async function disconnect(hostInfo) {
 ...
 */
export async function retryWrapper(func, hostInfo, argMaxRetry, argDuration) {
